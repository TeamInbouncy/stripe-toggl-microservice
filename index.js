...
// // ---------- FIXED: Toggl Time Entry Function with Rate Limiting ----------
...
//     console.log(`📊 [TOGGL-TIME] Found ${entries.length} time entries for project ${projectId}`);

//     if (entries.length === 0) {
//       console.log('⚠️ [TOGGL-TIME] No time entries found for this project in the specified period');
//       return 0;
//     }

//     let billableEntries = 0;

//     console.log('🔍 [TOGGL-TIME] Checking entries:');
//     entries.forEach((entry, index) => {
//       const isBillable = entry.billable === true;
//       const hasDuration = typeof entry.duration === 'number' && entry.duration > 0;
      
//       console.log(`   ${index + 1}. Billable: ${isBillable}, Duration: ${entry.duration}, Desc: "${entry.description}"`);
      
//       if (isBillable && hasDuration) {
//         totalSeconds += entry.duration;
//         billableEntries++;
//         console.log(`   ✅ COUNTED: ${entry.duration} seconds`);
//       }
...
//     console.log(`📈 [TOGGL-TIME] SUMMARY:`);
//     console.log(`   - Total entries for project: ${entries.length}`);
//     console.log(`   - Billable entries: ${billableEntries}`);
//     console.log(`   - Total seconds: ${totalSeconds}`);
//     console.log(`   - Total hours: ${(totalSeconds / 3600).toFixed(2)}`);
...
//       console.error('⏰ [TOGGL-TIME] Please wait or upgrade Toggl plan');
//       return 0;
//     }
...
//     if (calculatedHours <= 0) {
//       console.log('⏭️ [REAL-TIME] No hours to sync');
//       return res.json({ 
//         success: true, 
//         synced: false, 
//         message: 'No hours to sync',
...
// ---------- FIXED: Toggl Time Entry Function with PROPER filtering ----------
...
async function fetchTogglBillableSecondsForProject(projectId, since, until) {
  ...
  try {
    ...
    // FIX: Client-side filtering (more reliable than project_ids parameter)
    const projectEntries = entries.filter(entry => 
      entry.project_id === parseInt(projectId)
    );

    console.log(`📊 [TOGGL-TIME] ${projectEntries.length} entries for project ${projectId}`);

    if (projectEntries.length === 0) {
      console.log('⚠️ [TOGGL-TIME] No time entries found for this project');
      
      // Debug: Show what projects we did find entries for
      const projectCounts = {};
      entries.forEach(entry => {
        const pid = entry.project_id;
        projectCounts[pid] = (projectCounts[pid] || 0) + 1;
      });
      console.log('🔍 [TOGGL-TIME] Entries found for projects:', projectCounts);
    ...
    projectEntries.forEach((entry, index) => {
      ...
      if (duration < 0) {
        // Timer is running - calculate duration from start time
        const startTime = new Date(entry.start);
        const currentTime = new Date();
        duration = Math.floor((currentTime - startTime) / 1000);
        console.log(`   ⏱️  Running timer: calculated ${duration}s from start time`);
      }
      
      const hasDuration = typeof duration === 'number' && duration > 0;
      
      console.log(`   ${index + 1}. Billable: ${isBillable}, Duration: ${duration}s, Desc: "${entry.description}"`);
      
      if (isBillable && hasDuration) {
        totalSeconds += duration;
        billableEntries++;
        console.log(`   ✅ COUNTED: ${duration} seconds`);
      } else if (!isBillable) {
        console.log(`   ❌ SKIPPED: Not billable`);
      } else if (!hasDuration) {
    ...
    console.log(`📈 [TOGGL-TIME] SUMMARY:`);
    console.log(`   - Total entries for project: ${projectEntries.length}`);
    console.log(`   - Billable entries: ${billableEntries}`);
    console.log(`   - Total seconds: ${totalSeconds}`);
    console.log(`   - Total hours: ${(totalSeconds / 3600).toFixed(2)}`);
...
// ---------- FIXED: Usage sync job with BATCH processing ----------
...
app.post('/jobs/sync-usage', async (req, res) => {
  ...
  try {
    ...
    // FIX: Use batch processing to avoid rate limits
    for (const mapping of mappings) {
      console.log('\n🔍 [SYNC] Processing mapping:');
      console.log('   - Customer:', mapping.stripe_customer_id);
      console.log('   - Company:', mapping.company_name);
      ...
      // FIX: Always look back 24 hours to catch recent entries
      const since = mapping.last_synced_at || new Date(now.getTime() - 24 * 60 * 60 * 1000);
      console.log(`⏰ [SYNC] Time range: ${since.toISOString()} to ${now.toISOString()}`);
...
// ---------- Real-time sync for single customer (IMPROVED) ----------
...
app.post('/sync-customer', async (req, res) => {
  ...
  const { customer_id, hours, force_sync } = req.body;
  ...
  try {
    ...
    // FIX: Use force_sync to get all recent hours
    let since;
    if (force_sync) {
      since = new Date(now.getTime() - 24 * 60 * 60 * 1000); // 24 hours back
      console.log('🔧 [REAL-TIME] FORCE SYNC - fetching all time from last 24 hours');
    } else {
      since = mapping.last_synced_at || new Date(now.getTime() - 24 * 60 * 60 * 1000);
    }
    ...
    const form = new URLSearchParams();
    form.append('event_name', STRIPE_METER_EVENT_NAME);
    form.append('payload[stripe_customer_id]', customer_id);
    form.append('payload[value]', calculatedHours.toFixed(2));
...
// ---------- NEW: Auto-sync endpoint for cron jobs ----------

app.post('/jobs/auto-sync', async (req, res) => {
  console.log('\n🤖 [AUTO-SYNC] Automated sync started');
  
  if (!USAGE_JOB_SECRET || req.query.secret !== USAGE_JOB_SECRET) {
    console.error('❌ [AUTO-SYNC] Unauthorized');
    return res.status(401).json({ error: 'Unauthorized' });
  }
  ...
  try {
    ...
    console.log(`📊 [AUTO-SYNC] Processing ${mappings.length} customers`);

    for (const mapping of mappings) {
      console.log(`\n🔍 [AUTO-SYNC] Processing: ${mapping.company_name}`);
      ...
      const hours = totalSeconds / 3600;
      console.log(`📈 [AUTO-SYNC] Found ${hours.toFixed(2)} hours for ${mapping.company_name}`);

      if (hours <= 0) {
        console.log('⏭️ [AUTO-SYNC] No hours to sync');
        results.push({
          company: mapping.company_name,
          hours: 0,
          synced: false
      ...
      try {
        console.log(`📤 [AUTO-SYNC] Sending ${hours.toFixed(2)}h to Stripe...`);
        ...
        console.log(`✅ [AUTO-SYNC] Successfully synced ${hours.toFixed(2)}h`);
        syncedCount += 1;
        ...
        console.error(`❌ [AUTO-SYNC] Stripe error for ${mapping.company_name}:`, stripeErr.response?.data);
        results.push({
          company: mapping.company_name,
          hours: hours.toFixed(2),
          synced: false,
    ...
    console.log(`✅ [AUTO-SYNC] Completed: ${syncedCount} customers synced`);
    res.json({
      success: true,
      synced_count: syncedCount,
      time_range: {
...
app.post('/debug/toggl-time', async (req, res) => {
  ...
  const { project_id, days = 1 } = req.body;
...
// ---------- NEW: Verify sync status ----------

app.post('/verify-sync', async (req, res) => {
  console.log('\n🔍 [VERIFY] Verifying sync status');
  ...
  try {
    ...
    // Check recent Stripe meter events for this customer
    ...
    res.json({
      ...
      recent_meter_events: meterEvents.data.map(evt => ({
        id: evt.id,
        timestamp: new Date(evt.timestamp * 1000).toISOString(),
        value: evt.payload.value,
        event_name: evt.event_name
      })),
      total_events: meterEvents.data.length,
      status: meterEvents.data.length > 0 ? 'SYNC_ACTIVE' : 'NO_RECENT_SYNC'
    });
