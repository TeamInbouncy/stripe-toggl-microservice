...
async function handleCheckoutSessionCompleted(session) {
  ...
  try {
    ...
    if (session.custom_fields) {
      for (const field of session.custom_fields) {
        const fieldKey = field.key ? String(field.key).toLowerCase() : '';
        if ((fieldKey.includes('company') || fieldKey.includes('business')) && field.text?.value) {
          companyName = field.text.value;
          break;
        }
      }
...
async function processSubscriptionWithCompanyName(subscriptionId, companyName) {
  try {
    ...
    if (!customerId || !priceItem) return;
    ...
    if (product.name) {
      const productParts = product.name.split('|');
      planName = productParts[1] ? productParts[1].trim() : product.name.trim();
    }
    ...
    console.log(`✅ Created: ${companyName} - ${planLabel}`);
  } catch (err) {
    console.error('❌ Subscription Process Error:', err);
  }
}
...
// ---------- Toggl Helpers ----------
async function findOrCreateTogglClient(companyName) {
  if (!TOGGL_WORKSPACE_ID) throw new Error('TOGGL_WORKSPACE_ID not set');
  ...
  try {
    ...
    console.error('❌ Toggl Client Error:', err.response?.data || err.message);
    throw err;
  }
}
...
async function findOrCreateTogglProject(clientId, projectName) {
  ...
  try {
    ...
    console.error('❌ Toggl Project Error:', err.response?.data || err.message);
    throw err;
  }
}
...
async function fetchTogglBillableSecondsForProject(projectId, since, until) {
  ...
  try {
    ...
    if (err.response?.status === 402 || err.response?.status === 429) {
      console.error('❌ Toggl Rate Limit - Please wait');
      return 0;
    }
    console.error('❌ Toggl Time Error:', err.response?.data || err.message);
    return 0;
  }
}

// ---------- FIXED: Efficient Sync Job (Last Hour Only) ----------
app.post('/jobs/sync-usage', async (req, res) => {
  if (!USAGE_JOB_SECRET || req.query.secret !== USAGE_JOB_SECRET) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  ...
  try {
    ...
    // FIX: Only look back 1 HOUR for recent time entries
    const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
    
    let syncedCount = 0;
    const results = [];

    console.log(`🔄 Syncing ${mappings.length} customers (last hour only)`);

    for (const mapping of mappings) {
      // Skip if no recent activity expected
      if (mapping.last_synced_at && mapping.last_synced_at > oneHourAgo) {
        continue;
      }
      ...
      try {
        ...
        if (hours > 0) {
          ...
          console.log(`✅ ${mapping.company_name}: ${hours.toFixed(2)}h`);
        }
      } catch (err) {
        console.error(`❌ ${mapping.company_name}:`, err.response?.data || err.message);
        results.push({
          company: mapping.company_name,
    ...
    console.log(`🎉 Sync Complete: ${syncedCount} customers updated`);
    res.json({ 
      status: 'ok', 
      synced: syncedCount,
      period: 'last_hour',
...
// ---------- Real-time Sync (Single Customer) ----------
app.post('/sync-customer', async (req, res) => {
  if (!USAGE_JOB_SECRET || req.query.secret !== USAGE_JOB_SECRET) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
// ---------- Health Check ----------
app.get('/', (req, res) => {
  res.send('Stripe → Toggl Sync Service Running');
});
